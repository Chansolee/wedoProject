...
listPageServer = function(cpage){
	...
	$.ajax({
		...
		success : function(res){
			href = "'insertnotice.jsp'";
			code=	'<section class="ftco-section">';
code +=		'<div class="container" style="border: 1px solid; border-radius: 10px; padding-top: 12px;" >';
//code +=  '<input name="insert" type="button"  value="글쓰기" class="action" > ';

/*code += '<button type="button" class="btn mb-2 btn-primary" onclick="location.href='+href+'">작성</button>';*/
...
code +=				'</div>';
code +=			'</div>';
code += '<button type="button" style="margin-left:1000px;" class="btn mb-2 btn-primary" onclick="location.href='+href+'">작성</button>';
code +=		'</div>';
code +=	'</section>';
